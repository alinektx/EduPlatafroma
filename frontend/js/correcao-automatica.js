    ...
    atualizarCartaoResultado(resultado, index) {
        ...
        if (resultCard) {
            ...
            if (actionsDiv && !actionsDiv.querySelector('.btn-info')) {
                ...
                pdfButton.onclick = () => abrirPDF(index);
...
document.addEventListener('DOMContentLoaded', function() {
    window.correcaoAutomatica = new CorrecaoAutomatica();
});
...
function abrirPDF(index) {
    if (window.correcaoAutomatica) {
        window.correcaoAutomatica.abrirPDF(index);
    }
}

function removeFile(index) {
    if (window.correcaoAutomatica) {
        window.correcaoAutomatica.removeFile(index);
    }
} 